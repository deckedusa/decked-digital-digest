...
Shopify discounts originally were totaled in Celigo and added as a fixed amount discount at the header level in NetSuite. This approach has created issues with refunds and tax calculations when orders are modified. We have transitioned to use Celigo's native integration that creates discount line items for line items with discounts in Shopify, while retaining a header-level discount for order-level discounts. This configuration allows for proper tax and refund calculations.
